...
While building my personal site, I wanted a smarter way to handle resumes—something version-controlled, customizable, and always up-to-date. That’s when I decided to build a dynamic resume generator using Next.js and React PDF. The idea came from wanting to maintain my resume as code - something I could version control, easily update, and generate consistently formatted PDFs from.
...
<Callout title="Note" icon="circle-check-big" variant="success">
  This API route leverages `renderToBuffer()` from `@react-pdf/renderer`, turning my React-based
  Resume component into a binary PDF response.
</Callout>

The `Resume` component is where the magic happens. It's a React PDF document that renders each section:

```tsx filename=resume.tsx
...
```tsx filename=experience-section.tsx
